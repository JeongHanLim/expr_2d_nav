...
class CycleGAN(nn.Module):
    ...
    def forward(self, state_1, state_2):
        self.latent_vector_1, mu_1, log_var_1 = self.enc_1(state_1)
        self.latent_vector_2, mu_2, log_var_2 = self.enc_2(state_2)
        ...
        return self.decode_state_1, self.decode_state_2, self.cycle_latent_vector_1, self.cycle_latent_vector_2, \
               self.latent_vector_1, self.latent_vector_2, self.discrim_1, self.discrim_2 ,mu_1, mu_2, log_var_1, log_var_2, \
               self.transfer_latent_vector_1, self.transfer_latent_vector_2
...
class CycleGANGen(nn.Module):
    ...
    def forward(self, state_1, state_2):
        latent_vector_1, _, _ = self.enc_1(state_1)
        latent_vector_2, _, _ = self.enc_2(state_2)
...
class Encoder(nn.Module):
    ...
    def forward(self, input_data):
        x = F.relu(self.enc_1(input_data))
        x = self.enc_2(x)
        x = F.relu(self.bn(x))
        mu = self.z_mu(x)
        log_var = self.z_sig(x)
        eps = torch.randn(mu.shape)
        z = mu + log_var.mul(0.5).exp_() * eps
        return z, mu, log_var
...
class Decoder(nn.Module):
    ...
    def forward(self, input_data):
        x = F.relu(self.dec_1(input_data))
        x = self.dec_2(x)
        x = F.relu(self.bn(x))
        x = self.dec_3(x)
        return x
...
class Discriminator(nn.Module):
    ...
    def forward(self, input_data):
        x = F.relu(self.dis_1(input_data))
        x = self.dis_2(x)
        x = F.relu(self.bn(x))
        x = self.dis_3(x)
        return x